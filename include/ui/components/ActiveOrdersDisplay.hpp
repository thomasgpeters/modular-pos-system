 ...
 * 
 * This component displays all active orders in a table format with
 * status information and action buttons.
 * 
 ...
 * @brief UI component for displaying active orders
 * 
 * The ActiveOrdersDisplay shows all orders that are currently active
 * (not completed or cancelled) in a table format with order details,
 * status information, and available actions.
 */
...
public:
     ...
     * @param maxOrders Maximum number of orders (0 = no limit)
     */
    void setMaxOrdersToDisplay(int maxOrders);
    
    /**
     * @brief Gets the maximum number of orders to display
    ...
    /**
     * @brief Checks if completed orders are shown
...
protected:
    ...
    /**
     * @brief Adds an order row to the table
